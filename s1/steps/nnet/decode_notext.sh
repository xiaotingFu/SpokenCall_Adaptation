...
if ! $skip_scoring ; then
  [ ! -x steps/score_kaldi_notext.sh ] && \
    echo "Not scoring because steps/score_kaldi_notext.sh does not exist or not executable." && exit 1;
  steps/score_kaldi_notext.sh $scoring_opts --cmd "$cmd" $data $graphdir $dir || exit 1;
fi
