...
if [ $step -le 1 ]; then
    ...
    steps/make_mfcc.sh --nj 30 --cmd "$train_cmd" $data_dir $data_dir/log $data_dir/data
...
if [ $step -le 2 ]; then
    ...
    #utils/subset_data_dir.sh $data_dir 5000 $data_dir_5k
...
if [ $step -le 3 ]; then
    ...
    echo '''                        Step 3 : tri1                                                    '''
    ####################################################################################################
    time steps/train_deltas.sh \
        --boost-silence $boost_sil --cmd "$train_cmd" $numLeavesTri1 $numGaussTri1 \
        $data_dir $lang ${exp_dir}/mono_ali ${exp_dir}/tri1
    steps/align_si.sh \
      --boost-silence $boost_sil --nj $nj --cmd "$train_cmd" $data_dir $lang ${exp_dir}/tri1 ${exp_dir}/tri1_ali
...
if [ $step -le 4 ]; then
    ...
    echo '''                        Step 4 : tri2                                                    '''
    ####################################################################################################
    time steps/train_deltas.sh --boost-silence $boost_sil --cmd "$train_cmd" $numLeavesTri1 $numGaussTri1 \
        $data_dir $lang ${exp_dir}/tri1_ali ${exp_dir}/tri2a
    steps/align_si.sh --boost-silence $boost_sil --nj $nj --cmd "$train_cmd" $data_dir $lang ${exp_dir}/tri2a ${exp_dir}/tri2a_ali
...
if [ $step -le 5 ]; then
    ...
    # align with SAT
    steps/align_fmllr.sh --boost-silence $boost_sil --nj $nj --cmd "$train_cmd" \
...
if [ $step -le 6 ]; then
    ...
    ####################################################################################################
    time steps/train_sat.sh --boost-silence $boost_sil --cmd "$train_cmd" \
        $numLeavesTri4 $numGaussTri4 $data_dir $lang ${exp_dir}/tri3a_ali ${exp_dir}/tri4a
    time steps/align_fmllr.sh --boost-silence $boost_sil --nj $nj --cmd "$train_cmd" \
